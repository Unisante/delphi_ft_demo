...
dft2_type2_zz0_id_selected <- dft2_dt_type2_m[record_id == record_id_selected, .(responses_participant =.N), keyby = .(variable, item)]
...
dft2_type2_zz1_id_selected <- dft2_type2_zz1[dft2_type2_zz0_id_selected, on = .(variable, item), responses_participant := responses_participant
][order(variable, 
...
cols_type3_no_opinion <- data_clean[, names(.SD), .SDcols = patterns('_type3_no_op')]
...
dft2_type3_zz0_id_selected <- data_clean[record_id == record_id_selected, .(
  item_name = names(.SD), 
  # item = lapply(.SD, sjlabelled::get_label),
  responses_participant = lapply(.SD, sum)
), .SDcols = c(cols_type3_in_data_clean)] 

dft2_type3_zz0_id_selected

dft2_type3_zz0_no_opinion_id_selected <- data_clean[record_id == record_id_selected, .(
  item_name = names(.SD), 
  # item = "Pas d'avis",
  responses_participant = lapply(.SD, function(x) {sum(!is.na(x))})
), .SDcols = c(cols_type3_no_opinion)]


dft2_type3_zz1_id_selected <- rbindlist(list(dft2_type3_zz0_id_selected, dft2_type3_zz0_no_opinion_id_selected))


## . dft2_type3_zz1_id_selected ---- 
## .. left join ---
# dft2_type3_zz1_id_selected <- dft2_type3_zz1[dft2_type3_zz1_id_selected, on = 'item_name']
dft2_type3_zz1_id_selected <- dft2_type3_zz1_id_selected[dft2_type3_zz1, on = 'item_name'] 
