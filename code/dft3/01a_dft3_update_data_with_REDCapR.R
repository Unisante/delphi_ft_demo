...
## Aim : Export data and metadata from a REDCap Project to RData (data.table format)
...
data.table::setDT(dft3_metadata)
...
dft3_data_redcapr_raw <- REDCapR::redcap_read_oneshot(
  redcap_uri = redcap_uri,
  token      = token,
...
str(dft3_data_redcapr_raw)

## . check structure of data -----------------------------------------
##  https://ouhscbbmc.github.io/OuhscMunge/
# remotes::install_github("OuhscBbmc/OuhscMunge")
OuhscMunge::verify_value_headstart(dft3_data_redcapr_raw)



## 2. save ----------------------------------------------------------
save(dft3_data_redcapr_raw, file = here::here('data', 'redcap_data_raw', 'dft3_data_redcapr_raw.RData'))
